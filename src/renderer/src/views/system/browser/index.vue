...
<script>
import { listBrowser, getBrowser, delBrowser, addBrowser, updateBrowser } from "@/api/system/browser";
  ...
  methods: {
    ...
    reset() {
      ...
      if (this.$refs.browserRef) {
        this.$refs.browserRef.resetFields();
      }
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    handleUpdate(row) {
      ...
      getBrowser(String(id)).then(response => {
    ...
    /** 提交按钮 */
    submitForm() {
      this.$refs.browserRef.validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateBrowser(this.form).then(response => {
            ...
            addBrowser(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除浏览器实例编号为"' + ids + '"的数据项？').then(() => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/browser/export', {
        ...this.queryParams
